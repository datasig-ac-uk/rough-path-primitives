...
public:
    ...
    operator()(Context const& ctx,
               Vector& vec,
               Accum const& value = Accum{}) const noexcept {
        static_assert(static_assert_fail<Strategy, Context, Vector>,
                      "rpp::ops::VectorSetConstant has no implementation for "
                      "this Strategy. "
...
auto vector_set_constant(
    ...
    typename Strategy::Accum value = typename Strategy::Accum{}) noexcept {
    using Op = VectorSetConstant<Strategy>;

    static_assert(
        Op::is_implemented,
        "The operation object \"VectorSetConstant\" that implements "
        "\"vector_set_constant\" is not implemented. This either means that "
        "the "
        ...
        "this operation:\n\n"
        "    #include "
        "<rpp/cpu/single_thread/operations/linalg/vector_set_constant.hpp>");
