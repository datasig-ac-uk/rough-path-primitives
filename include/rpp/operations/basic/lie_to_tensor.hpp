...
template <typename Strategy,
          typename TensorBatchOut,
          typename LieBatchIn,
          typename Matrix,
          typename TensorBasis,
          typename LieBasis>
auto lie_to_tensor(Strategy const& strategy,
                   typename Strategy::LaunchConfig config,
                   TensorBatchOut const& out,
                   LieBatchIn const& arg,
                   TensorBasis const& tensor_basis,
                   LieBasis const& lie_basis,
                   typename Strategy::Index num_batches,
                   Matrix const& matrix) noexcept {
    static constexpr auto implementation =
        sparse::matrix_format_v<Matrix> == sparse::MatrixFormat::CSR
    ...
    static_assert(
        Op::is_implemented,
        "The operation object \"LieToTensor\" that implements "
        "\"lie_to_tensor\" "
        ...
        "    #include "
        "<rpp/cpu/single_thread/operations/basic/lie_to_tensor.hpp>");

    return strategy.template launch<Op>(
        std::move(config),
        std::make_tuple(out, arg),
        make_basis_pack(tensor_basis, lie_basis),
        num_batches,
        matrix);
}

// template <typename Strategy> class LieToTensor<Strategy,
// L2TImplementationType::CommutatorExpansion> {
//     using Context = typename Strategy::Context;
// public:
//
//     template <typename Basis>
...
//     RPP_HOST_DEVICE
//     void operator()(Context const& ctx, TensorOut& out, LieIn const& arg)
//     const noexcept {
//         static_assert(
