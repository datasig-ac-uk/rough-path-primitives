...
void insert_commutator(Builder& builder,
                       Basis const& basis,
                       typename Basis::Index left_key,
                       typename Basis::Index right_key) {
    ...
    const auto left_offset =
        basis::tensor_degree_size<Index>(basis.width, left_degree);
    const auto right_offset =
        basis::tensor_degree_size<Index>(basis.width, right_degree);

    for (Index i = 0; i < left_frame.size; ++i) {
        for (Index j = 0; j < right_frame.size; ++j) {
            builder.get_scalar(left_frame.index[i] * right_offset +
                               right_frame.index[j]) +=
                left_frame.data[i] * right_frame.data[j];

            builder.get_scalar(right_frame.index[i] * left_offset +
                               left_frame.index[j]) -=
...
make_l2t_matrix(LieBasis const& basis) {
    ...
    return {std::move(data),
            ...
            nnz,
            basis.size(),
            basis::tensor_dimension<Index>(basis.width, basis.depth)};
}
