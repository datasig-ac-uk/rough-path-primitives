...
class Span : detail::ExtentHolder<N> {
    ...
    static constexpr size_t subspan_extent() noexcept {
        static_assert(
            Offset <= N && (OtherExtent == dynamic_extent || OtherExtent <= N - Offset),
...
public:
    ...
    RPP_HOST_DEVICE
    constexpr Span(T (&array)[ArraySize]) noexcept
        : Extent(ArraySize), data_(array)
    {}

    template <size_t ArraySize>
    RPP_HOST_DEVICE
    constexpr Span(std::array<T, ArraySize> array) noexcept
    ...
    template <typename I>
    RPP_HOST_DEVICE RPP_NODISCARD
    constexpr reference operator[](I idx) const noexcept {
        return data()[idx];
    }
    ...
    RPP_HOST_DEVICE RPP_NODISCARD
    constexpr Span<T, subspan_extent<Count>> first() const noexcept {
    ...
    template <size_t Count>
    RPP_HOST_DEVICE RPP_NODISCARD
    constexpr Span<T, subspan_extent<Count>>
    last() const noexcept {
    ...
    RPP_HOST_DEVICE RPP_NODISCARD
    constexpr Span<T, subspan_extent<Count, Offset>>
