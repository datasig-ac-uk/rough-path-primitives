...
inline constexpr unsigned dynamic_block_size =
...
public:
    ...
    static constexpr bool static_small_block =
        static_block_size < Strategy::warp_size;

    static constexpr bool is_nothrow =
        std::is_integral_v<Accum> || std::is_floating_point_v<Accum>;
...
public:
    ...
    RPP_DEVICE static void sync_warp() noexcept { __syncwarp(); }
    ...
    RPP_DEVICE T reduce(T val, Fn&& fn) const noexcept {
        if constexpr (static_block_size == dynamic_block_size ||
                      static_block_size >= Strategy::warp_size) {
            ...
            for (unsigned i = Strategy::block_size / 2; i > 0; i /= 2) {
                val = fn(val,
                         __shfl_down_sync(0xFFFFFFFF, val, i, num_threads()));
            }
            return val;
        }
    }
...
struct BlockSizeHolder<dynamic_block_size> {
    ...
    constexpr BlockSizeHolder(unsigned size) noexcept : block_size(size) {}
};
} // namespace detail


template <typename Accum_,
          unsigned BlockSize = dynamic_block_size,
          unsigned MaxBlockSize = 256,
...
struct BlockStrategy : public detail::BlockSizeHolder<BlockSize> {
    ...
    static constexpr unsigned warp_count =
        (block_size + warp_size - 1) / warp_size;
    static constexpr unsigned max_warp_count =
        (max_block_size + warp_size - 1) / warp_size;
    ...
    static_assert(is_pow_2(block_size) && block_size <= max_block_size &&
                      (block_size % warp_size) == 0,
                  "invalid block configuration: block size must be a power of "
                  "2, within max block size, and divisible by warp size");

    using BlockReduceArray = Accum[max_warp_count];

    RPP_HOST RPP_NODISCARD constexpr unsigned
    launch_block_size() const noexcept {
    ...
    RPP_HOST_DEVICE RPP_NODISCARD static constexpr Index
    objects_per_block() noexcept {
        if constexpr (BlockSize < warp_size) {
    ...
    RPP_HOST_DEVICE RPP_NODISCARD static constexpr Index
    threads_per_object() noexcept {
        return block_size;
    }

    RPP_HOST_DEVICE RPP_NODISCARD static constexpr Index
    object_index(unsigned block_index, unsigned thread_index) noexcept {
        if constexpr (BlockSize < warp_size) {
            return static_cast<Index>(block_index) * objects_per_block() +
    ...
    RPP_DEVICE static constexpr Context
    make_context(std::byte* smem_bytes) noexcept {
        if constexpr (BlockSize < warp_size) {
    ...
    RPP_HOST Error<char const*> launch(LaunchConfig&& config,
                                       Batches&& batches,
                                       Bases&& bases,
                                       Index batch_size,
...
inline constexpr bool is_block_strategy_v<
    BlockStrategy<Accum, BlockSize, MaxBlockSize, Architecture>> = true;


template <typename Accum_,
          unsigned BlockSize,
          unsigned MaxBlockSize,
          typename Architecture_>
template <typename Op, typename Batches, typename Bases, typename... Extras>
Error<char const*>
BlockStrategy<Accum_, BlockSize, MaxBlockSize, Architecture_>::launch(
    ...
    config.gridDim = dim3{
        static_cast<unsigned>((batch_size + num_objects - 1) / num_objects)};
