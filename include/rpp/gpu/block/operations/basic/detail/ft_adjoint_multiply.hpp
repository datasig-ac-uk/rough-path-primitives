...
template <typename Accum,
          ...
          typename Basis,
          typename ArgIndexFn>
RPP_DEVICE static Accum
adjoint_low_degree_reduce(Context const& ctx,
                          TensorOp const& op,
                          TensorArg const& arg,
                          typename TensorOp::Degree op_degree_min,
                          typename TensorOp::Degree op_degree_max,
                          Basis const& basis,
                          ArgIndexFn&& arg_idx) noexcept {
    ...
    for (Index i = begin + static_cast<Index>(ctx.thread_rank()); i < end;
         i += ctx.num_threads()) {
...
#endif // RPP_GPU_BLOCK_OPERATIONS_BASIC_DETAIL_FT_ADJOINT_MULTIPLY_HPP