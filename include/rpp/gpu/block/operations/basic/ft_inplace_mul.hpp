    ...
    RPP_DEVICE void operator()(Context const& ctx,
        ...
        for (Degree out_deg = lhs.max_degree(); out_deg > low_range_degree;
            ...
            for (auto elt_idx =
                ...
                const auto acc = gpu::block::ft_multiply_loop_with_degree(
                    ctx, lhs, rhs, elt_idx, out_deg, basis);
                lhs[elt_idx] = static_cast<Scalar>(beta * acc);
            }
            ctx.sync();
        }

        auto const low_range_begin = lhs.begin_index();
        ...
        if (active) {
            acc = gpu::block::ft_multiply_loop_with_degree(
                ctx, lhs, rhs, elt_idx, basis.degree_linear(elt_idx), basis);
        }
