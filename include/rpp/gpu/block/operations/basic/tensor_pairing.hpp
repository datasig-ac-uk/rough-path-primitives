...
class TensorPairing<
    gpu::strategies::
        BlockStrategy<Accum, BlockSize, MaxBlockSize, Architecture>>
...
public:
    ...
    RPP_DEVICE void operator()(Context const& ctx,
        ...
        Accum val{0};
        for (auto i = begin + ctx.thread_rank(); i < end;
             i += ctx.num_threads()) {
...
#endif // RPP_GPU_BLOCK_OPERATIONS_BASIC_TENSOR_PAIRING_HPP