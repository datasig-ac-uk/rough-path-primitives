    ...
    RPP_DEVICE void operator()(Context const& ctx,
        ...
        for (Index elt_idx = basis.start_of_degree(out_deg) +
            ...
            const auto op_min_deg =
                std::max(static_cast<Degree>(arg.min_degree() - elt_degree),
                         op.min_degree());
            const auto op_max_deg =
                std::min(static_cast<Degree>(arg.max_degree() - elt_degree),
                         op.max_degree());
            ...
            for (Degree op_deg = op_min_deg; op_deg < op_max_deg; ++op_deg) {
                ...
                for (Index op_idx = basis.start_of_degree(op_deg);
                     ...
                     ++op_idx) {
                    elt += Accum{op[op_idx]} *
                        Accum{arg[op_idx * stride + elt_idx]};
                }
            }
...
#endif // RPP_GPU_BLOCK_OPERATIONS_BASIC_FT_ADJ_LMUL_HPP
