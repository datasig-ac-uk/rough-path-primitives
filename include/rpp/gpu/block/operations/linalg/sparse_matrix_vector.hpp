#ifndef RPP_GPU_BLOCK_OPERATIONS_LINALG_SPARSE_MATRIX_VECTOR_HPP
#define RPP_GPU_BLOCK_OPERATIONS_LINALG_SPARSE_MATRIX_VECTOR_HPP
...
public:
    ...
    operator()(Context const& ctx,
        ...
        for (Index row = warp_idx; row < static_cast<Index>(matrix.rows());
             row += ctx.num_warps()) {
            Accum acc{0};
            const auto begin = matrix.offset(row);
            const auto end = matrix.offset(row + 1);

            for (auto entry = begin + warp_lane; entry < end;
                 entry += Strategy::warp_size) {
...
private:
    ...
    template <typename VectorOut,
              ...
              typename OffsetIter>
    RPP_DEVICE void
    atomic_out(Context const& ctx,
               VectorOut& out,
               VectorArg const& arg,
               Matrix<DataIter, IndexIter, OffsetIter> const& matrix,
               Accum alpha) const noexcept {
        ...
        for (Index idx = ctx.thread_rank(), end = matrix.nnz(); idx < end;
             idx += ctx.num_threads()) {
            auto my_row = matrix.inner_index(idx);
            auto my_col =
                algo::index_upper_bound(
                    matrix.offsets(), Index(0), matrix.outer_dim() + 1, idx) -
                1;
    ...
    template <typename VectorOut,
              ...
              typename IndexIter,
              typename OffsetIter>
    RPP_DEVICE static void
    block_wide(Context const& ctx,
               VectorOut& out,
               VectorArg const& arg,
               Matrix<DataIter, IndexIter, OffsetIter> const& matrix,
               Accum alpha) noexcept {
        ...
        for (Index row_block_begin = row_begin; row_block_begin < row_end;
             row_block_begin += ctx.num_threads()) {
            scratch[ctx.thread_rank()] = Accum{0};
            ctx.sync();

            for (Index col = col_begin; col < col_end; ++col) {
                auto [lower, upper] = matrix.offsets(col);
                auto block_col_begin = algo::index_lower_bound(
                    matrix.indices(), lower, upper, row_block_begin);
                auto block_col_end = algo::index_lower_bound(
                    matrix.indices(),
                    block_col_begin,
                    upper,
                    row_block_begin + ctx.num_threads());
                ...
                if (my_offset < block_col_end) {
                    ...
                    BlockAtomic out_ref{
                        scratch[my_row - matrix.offset(block_col_begin)]};
                    out_ref.fetch_add(mat_val * vec_val,
...
#endif // RPP_GPU_BLOCK_OPERATIONS_LINALG_SPARSE_MATRIX_VECTOR_HPP