#ifndef RPP_CPU_OPERATIONS_SINGLE_THREAD_BASIC_VECTOR_ADD_HPP
#define RPP_CPU_OPERATIONS_SINGLE_THREAD_BASIC_VECTOR_ADD_HPP
...
public:
    ...
    void operator()(Context const &ctx, VectorOut &out, VectorLhs const &lhs, VectorRhs const &rhs,
        ...
        const auto min_deg = std::max({out.min_degree(), lhs.min_degree(), rhs.min_degree()});
        const auto max_deg = std::min({out.max_degree(), lhs.max_degree(), rhs.max_degree()});
...
#endif //RPP_CPU_OPERATIONS_SINGLE_THREAD_BASIC_VECTOR_ADD_HPP
