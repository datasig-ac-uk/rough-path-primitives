...
public:
    ...
    RPP_HOST_DEVICE RPP_NODISCARD decltype(auto)
...
struct SingleThreadStrategy {
    ...
    using Context =
        detail::ThreadContext<SingleThreadStrategy<Accum_, Architecture_>>;
    ...
    RPP_HOST Error<std::string> launch(LaunchConfig launch_config,
                                       Batches batches,
                                       Bases bases,
                                       Index batch_size,
...
Error<std::string> SingleThreadStrategy<Accum_, Architecture_>::launch(
    ...
    auto extras_mapped =
        map_data_args<std::tuple>(mapper, std::forward<Extras>(extras)...);
    if (!extras_mapped) {
    ...
    auto result = catch_exceptions([&, op = Op{}] {
        for (Index idx = 0; idx < batch_size; ++idx) {
            ops::invoke(
                op,
                ctx,
                [&](auto& batch) { return batch.view(bases, idx); },
                batches,
                extras_mapped.value());
        }
    });
