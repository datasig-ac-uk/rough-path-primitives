#ifndef RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_TENSOR_PAIRING_HPP
#define RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_TENSOR_PAIRING_HPP
...
class TensorPairing<cpu::strategies::SingleThreadStrategy<Accum, Architecture>>
    : public BaseOperation<
          cpu::strategies::SingleThreadStrategy<Accum, Architecture>> {
...
public:
    ...
    void operator()(Context const& ctx,
                    Scalar& out,
                    TensorFunc const& functional,
                    TensorArg const& arg) const noexcept {
        ignore_unused(ctx);
        auto const& basis = functional.basis();
        const auto min_degree =
            std::max(functional.min_degree(), arg.min_degree());
        const auto max_degree =
            std::min(functional.max_degree(), arg.max_degree());

        Accum tmp{0};
        for (auto i = basis.start_of_degree(min_degree);
             i < basis.end_of_degree(max_degree);
             ++i) {
...
#endif // RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_TENSOR_PAIRING_HPP