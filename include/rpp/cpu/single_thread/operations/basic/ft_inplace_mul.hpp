#ifndef RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_FT_INPLACE_MUL_HPP
#define RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_FT_INPLACE_MUL_HPP
...
public:
    ...
    void operator()(Context const& ctx,
        ...
        for (Degree out_degree = lhs.max_degree(); out_degree >= out_min_degree;
             --out_degree) {
            auto out_frag = lhs.degree_view(out_degree);

            if (rhs.min_degree() == 0) {
                for (Index i = 0; i < out_frag.size(); ++i) {
                    out_frag[i] = static_cast<Scalar>(
                        beta * Accum{out_frag[i]} * Accum{rhs[0]});
                }
            }
            else {
                for (Index i = 0; i < out_frag.size(); ++i) {
            ...
            const auto lhs_degree_min =
                std::max(lhs.min_degree(),
                         static_cast<Degree>(out_degree - rhs.max_degree()));
            const auto lhs_degree_max = static_cast<Degree>(
                out_degree - std::max(Degree{1}, rhs.min_degree()));
            if (lhs_degree_max < lhs_degree_min) {
                continue;
            }

            for (Degree lhs_degree = lhs_degree_max;
                 lhs_degree >= lhs_degree_min;
                 --lhs_degree) {
                const auto rhs_degree =
                    static_cast<Degree>(out_degree - lhs_degree);
                if (!lhs.has_degree(lhs_degree) ||
                    !rhs.has_degree(rhs_degree)) {
                    continue;
                }
...
#endif // RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_FT_INPLACE_MUL_HPP