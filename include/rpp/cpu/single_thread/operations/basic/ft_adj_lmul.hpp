...
public:
    ...
    void operator()(Context const& ctx,
         ...
         * arguments in terms of which combination of degrees we access and in
         * what order these combinations appear. For instance, we might major
         * the output degree, in which case we compute all of the terms of a
         * given odeg before moving on to the next. This gives some notion of
         * locality in the output, but not in either of the input arguments.
         * Alternatively, we might major the operator degree or the argument
         * degree. There are probably scenarios where each choice of major is
         * optimal, but I don't yet understand the relationships. However, it is
         * clear that the argument degree will usually be the largest at play at
         * any given time, so it is reasonable to major the argument degree.
         *
        ...
        for (Degree arg_degree = arg_max_degree; arg_degree >= arg_min_degree;
             --arg_degree) {
            auto out_min_degree =
                std::max(arg_degree - op.max_degree(), out.min_degree());
            auto out_max_degree =
                std::min(arg_degree - op.min_degree(), out.max_degree());
...
#endif // RPP_CPU_SINGLE_THREAD_OPERATIONS_BASIC_FT_ADJ_LMUL_HPP
