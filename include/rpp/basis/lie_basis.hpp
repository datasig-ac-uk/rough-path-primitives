...
struct StandardLieBasisOrder {
    ...
    operator()(Left const& left, Right const& right) const noexcept {
        auto& [a, b] = left;
        auto& [c, d] = right;
        return a < c || (a == c && b < d);
    }
 ...
 * @class LieBasis
 *
 * Represents a Lie algebra basis object used for symbolic and numerical
 * computations. The class encapsulates a set of basis elements and provides
 * operations such as linear combination, product, and commutator evaluation. It
 * supports both dense representations and sparse storage, offering flexible
 * access to basis vectors and their algebraic relations.
 */
template <typename Architecture_,
          typename Ordering = detail::StandardLieBasisOrder>
struct LieBasis : GradedBasis<Architecture_, LieBasisTag>, Ordering {
    ...
    explicit constexpr LieBasis(Degree width,
                                Degree depth,
    ...
    key_to_idx(Index key) const noexcept {
        return key - 1;
    }
    RPP_HOST_DEVICE RPP_NODISCARD constexpr Index
    idx_to_key(Index idx) const noexcept {
        return idx + 1;
    }

    template <typename CmpLeft, typename CmpRight>
    RPP_HOST_DEVICE RPP_NODISCARD constexpr bool
    compare(CmpLeft const& left, CmpRight const& right) const noexcept {
        return static_cast<Ordering const&>(*this)(left, right);
    }

    constexpr Index find_bracket(Index left,
                                 Index right,
                                 Degree degree_hint = 0) const noexcept {
