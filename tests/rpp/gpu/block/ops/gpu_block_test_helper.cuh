...
#define RPP_REQUIRE_CUDA_DEVICE()                                                              \
    do {                                                                                       \
        ...
        const cudaError_t rpp_cuda_err = cudaGetDeviceCount(&rpp_cuda_device_count);            \
        if (rpp_cuda_err == cudaErrorNoDevice || rpp_cuda_err == cudaErrorInsufficientDriver || \
            rpp_cuda_err == cudaErrorSystemDriverMismatch) {                                    \
            GTEST_SKIP() << "No CUDA device is available";                                     \
        }                                                                                      \
        ASSERT_EQ(rpp_cuda_err, cudaSuccess)                                                    \
            << "cudaGetDeviceCount failed: " << cudaGetErrorString(rpp_cuda_err);              \
...
struct GpuBlockTestHelper {
    ...
    struct RandomScalar {
        ...
        Scalar operator()(std::size_t index) const
        {
            ...
            thrust::uniform_real_distribution<Scalar> dist(Scalar{-1}, Scalar{1});
            rng.discard(index);
            return scale * dist(rng);
        }
    };

    [[nodiscard]] static HostVector<Index> make_degree_begin(Degree width, Degree depth)
    {
    ...
    [[nodiscard]] static HostVector<Scalar> make_batch(
        unsigned seed,
        Basis const& basis,
        Scalar scale = Scalar{1}
    )
    {
        HostVector<Scalar> result(static_cast<std::size_t>(tensor_count * basis.size()));
    ...
    [[nodiscard]] static HostVector<Scalar> make_zero_batch(Basis const& basis)
    {
        HostVector<Scalar> result(static_cast<std::size_t>(tensor_count * basis.size()));
    ...
    template <typename T>
    [[nodiscard]] static HostVector<T> copy_to_host(DeviceVector<T> const& data)
    {
    ...
    {
        return rpp::make_graded_vector_batch(host_data(data), basis.size(), basis, Degree{0}, basis.depth);
    }
    ...
    {
        return rpp::make_graded_vector_batch(device_data(data), basis.size(), basis, Degree{0}, basis.depth);
    }
    ...
    {
        return rpp::make_tensor_batch(host_data(data), basis.size(), basis, Degree{0}, basis.depth);
    }
    ...
    {
        return rpp::make_tensor_batch(device_data(data), basis.size(), basis, Degree{0}, basis.depth);
    }
    ...
    [[nodiscard]] static auto device_scalar_batch(DeviceVector<Scalar> const& data)
    {
        return rpp::make_scalar_batch(device_data(data));
    }
    ...
    )
    {
        ...
        auto const scale = std::max(Scalar{1}, std::max(std::abs(actual), std::abs(expected)));
        auto const tolerance = std::max(absolute_tolerance, relative_tolerance * scale);
    ...
    static void expect_near(
        HostVector<Scalar> const& actual,
    ...
    )
    {
        ...
        for (std::size_t i = 0; i < actual.size(); ++i) {
            ...
            auto const scale = std::max(Scalar{1}, std::max(std::abs(actual[i]), std::abs(expected[i])));
            auto const tolerance = std::max(absolute_tolerance, relative_tolerance * scale);
            EXPECT_LE(difference, tolerance)
                << "at coefficient " << i
