...
TYPED_TEST_SUITE(GpuBlockFtMulTypedTests,
                 rpp::tests::TypedGpuAdjointTestTypes,
                 rpp::tests::TypedScalarAccumNameGenerator);

TYPED_TEST(GpuBlockFtMulTypedTests, MatchesHostReferenceForSingleElementBatches) {
...
TYPED_TEST(GpuBlockFtMulTypedTests, UnitActsAsTwoSidedIdentity) {
    RPP_REQUIRE_CUDA_DEVICE();
...
TYPED_TEST(GpuBlockFtMulTypedTests, RespectsTruncatedOperandAndOutputViews) {
    RPP_REQUIRE_CUDA_DEVICE();
