...
protected:
    ...
    [[nodiscard]] static auto make_csr(
        Vector const &values,
        ...
        return rpp::sparse::make_csr_matrix(
            values.data(),
            indices.data(),
            offsets.data(),
    ...
    [[nodiscard]] static auto make_csc(
        Vector const &values,
        std::vector<Index> const &indices,
        std::vector<Index> const &offsets,
        Index rows,
        ...
        return rpp::sparse::make_csc_matrix(
            values.data(),
            indices.data(),
            offsets.data(),
            static_cast<Index>(values.size()),
...
TEST_F(SparseMatrixVectorProductTests, AppliesCscMatrix) {
    ...
    auto const matrix = make_csc(values, indices, offsets, out_basis.size(), arg_basis.size());
...
TEST_F(SparseMatrixVectorProductTests, AppliesToLogicalViewRanges) {
    ...
    auto const matrix = make_csr(values, indices, offsets, basis.size_of_degree(1), basis.size_of_degree(2));
...
TEST_F(SparseMatrixVectorProductTests, KernelWrapperMatchesDirectOperation) {
    ...
    const auto err = rpp::ops::sparse_matrix_vector_product(
        strategy,
        {},
        out_batch,
        arg_batch,
