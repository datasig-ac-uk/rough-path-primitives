...
struct PolynomialTensorHelper {
    ...
    [[nodiscard]] static Monomial make_monomial(char letter,
                                                std::size_t position) {
        return Monomial(Indeterminate(letter, position));
    }

    [[nodiscard]] static Scalar
    make_scalar(std::initializer_list<
                std::tuple<std::initializer_list<std::pair<char, std::size_t>>,
                           std::int32_t,
                           std::int32_t>> coeffs) {
        Scalar result;
    ...
    [[nodiscard]] static std::vector<Index> make_degree_begin(Degree width,
                                                              Degree depth) {
    ...
    [[nodiscard]] static std::size_t symbol_index(Basis const& basis,
                                                  Degree degree,
                                                  Index level_index) noexcept {
        return static_cast<std::size_t>(basis.start_of_degree(degree) +
                                        level_index);
    }

    [[nodiscard]] static Scalar
    symbol(char marker, Basis const& basis, Degree degree, Index level_index) {
        return make_scalar(
            {{{{marker, symbol_index(basis, degree, level_index)}}, 1, 1}});
    }

    [[nodiscard]] static std::vector<Scalar> make_tensor(char marker,
                                                         Basis const& basis) {
        std::vector<Scalar> result;
        result.reserve(static_cast<std::size_t>(basis.size()));

        for_each_index(
            basis, [&result, marker, &basis](Degree degree, Index level_index) {
    ...
    unpack_level_index(Basis const& basis, Degree degree, Index level_index) {
    ...
    reverse_index(Basis const& basis, Degree degree, Index index) {
