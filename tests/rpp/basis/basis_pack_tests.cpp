...
struct TensorBasisData {
    ...
    TensorBasisData(Degree width, Degree depth)
        ...
        for (Degree d = 1; d <= depth + 1; ++d) {
            degree_begin[static_cast<std::size_t>(d)] = Index{1} +
                static_cast<Index>(width) *
                    degree_begin[static_cast<std::size_t>(d - 1)];
        }
    }
};
...
TEST(BasisPackTests, RetrievesPlainBasisByExactTag) {
    ...
    auto const& basis =
        rpp::basis::get_basis(rpp::basis::TensorBasisTag{}, pack);
...
TEST(BasisPackTests, RetrievesRoleAnnotatedBases) {
    ...
    auto const pack =
        rpp::basis::make_basis_pack(rpp::basis::in(input_basis_data.basis),
                                    rpp::basis::out(output_basis_data.basis));

    auto const& input_basis = rpp::basis::get_basis(
        rpp::basis::InputBasisTag<rpp::basis::TensorBasisTag>{}, pack);
    auto const& output_basis = rpp::basis::get_basis(
        rpp::basis::OutputBasisTag<rpp::basis::TensorBasisTag>{}, pack);
...
TEST(BasisPackTests, RetrievesIndexedBasesOfSameType) {
    ...
    auto const& first_basis = rpp::basis::get_basis(
        rpp::basis::IndexedBasisTag<0, rpp::basis::TensorBasisTag>{}, pack);
    auto const& second_basis = rpp::basis::get_basis(
        rpp::basis::IndexedBasisTag<1, rpp::basis::TensorBasisTag>{}, pack);
...
TEST(BasisPackTests, NestedAnnotationsPreserveExistingTag) {
    ...
    auto const annotated_basis =
        rpp::basis::in(rpp::basis::idx<1>(basis_data.basis));

    static_assert(
        std::is_same_v<
            typename decltype(annotated_basis)::BasisTag,
            rpp::basis::InputBasisTag<
...
TEST(BasisPackTests, UniquenessCheckAllowsDistinctTagsAndRejectsDuplicates) {
    EXPECT_TRUE((rpp::basis::detail::check_unique(rpp::basis::TensorBasisTag{},
                                                  rpp::basis::LieBasisTag{})));
